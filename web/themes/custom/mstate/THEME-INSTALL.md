...
## ABOUT MSTATE THEME - something

For Drupal 10, MState uses the Stable9 theme starting place, and is meant to expand on that to theme this website.
...
Working with MState requires a few things: 
...
```
(d) cd into the folder for MState and you are ready to start running the theme. The theme can be ran in multiple different ways from the theme root:
